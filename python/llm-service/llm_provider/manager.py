...
from .base import (
    LLMProvider, LLMProviderRegistry, CompletionRequest, CompletionResponse,
    ModelTier, CacheManager, RateLimiter, TokenUsage
)
...
class LLMManager:
        ...
        for name, config in providers_config.items():
            ...
            try:
                ...
                self.registry.register_provider(
                    name, 
                    provider,
                    is_default=(name == config.get('default'))
                )
    ...
    async def complete(
        ...
        **kwargs
        ...
        if self.cache and not request.stream:
            ...
            if cached_response:
                self.logger.info(f"Cache hit for request (hit rate: {self.cache.hit_rate:.2%})")
    ...
    def _get_fallback_provider(
        ...
        for pref in tier_prefs:
            ...
            if provider_name != failed_provider and provider_name in self.registry.providers:
    ...
    def _update_usage_tracking(self, request: CompletionRequest, response: CompletionResponse):
        ...
        for name, provider in self.registry.providers.items():
            status[name] = {
                ...
                'rate_limit': {
                    ...
                    if name in self.rate_limiters else None
                }
            }
