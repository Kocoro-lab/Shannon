...
class ProviderManager:
    ...
    def select_model(
        ...
        if specific_model:
            ...
            logger.warning(
                f"Requested model '{specific_model}' not available, falling back to tier selection"
            )
    ...
    async def generate_completion(
        ...
        **kwargs,
        ...
        result = await provider.generate_completion(
            messages=messages, model=model_id, **kwargs
        )
        ...
        if session_id and "usage" in result:
            ...
            logger.info(
                f"Session {session_id} token usage: {self.session_tokens[session_id]}"
            )
