...
LEAD_DECISION_SCHEMA = {
    "format": {
        ...
        "schema": {
                        ...
                        "properties": {
                            "type": {
                                ...
                                "enum": [
                                    ...
                                    "file_read", "shutdown_agent", "noop",
                                    "done", "reply", "synthesize",
                                ],
                            },
                            "task_id": {"type": "string"},
            ...
            "required": ["decision_summary", "actions"],
            "additionalProperties": False,
        },
    }
}
...
async def lead_decide(request: Request, body: LeadDecisionRequest) -> LeadDecisionResponse:
    ...
    try:
        ...
        if body.event.type == "human_input":
            ...
            if summary:
                event_lines.append(f"  Summary: {str(summary)[:10000]}")
        ...
        try:
            ...
            logger.warning(
                f"Lead returned non-JSON: error={e}, stop_reason={stop_reason}, "
