...
class ComplexityRequest(BaseModel):
    ...
    available_tools: Optional[List[str]] = Field(default_factory=list, description="Available tools")
...
class ComplexityResponse(BaseModel):
    ...
    reasoning: str
    source: Optional[str] = Field(default="heuristic", description="'model' when provider used, else 'heuristic'")
    provider: Optional[str] = Field(default=None, description="Provider used when source=model")
    raw_output: Optional[str] = Field(default=None, description="Raw model output (debug only)")
...
def _heuristic_analysis(body: ComplexityRequest) -> ComplexityResponse:
    ...
    if any(word in query_lower for word in ["calculate", "compute", "multiply", "divide", "add", "subtract"]):
        mode = "standard"; score = 0.4; agents = 1; tokens = 200; cost = 0.002
        capabilities = ["calculation", "tool_use"]; reasoning = "Calculation task requiring tool execution"
    elif any(word in query_lower for word in ["simple", "what is", "define", "list"]) or query_length < 50:
        mode = "simple"; score = 0.2; agents = 0; tokens = 100; cost = 0.001
        capabilities = ["basic_qa"]; reasoning = "Simple query that can be answered directly"
    elif any(word in query_lower for word in ["analyze", "compare", "explain", "describe"]):
        mode = "standard"; score = 0.5; agents = 1; tokens = 500; cost = 0.005
        capabilities = ["analysis", "reasoning"]; reasoning = "Standard analysis task requiring single agent"
    elif any(word in query_lower for word in ["implement", "design", "create", "build"]) or query_length > 200:
        mode = "complex"; score = 0.8; agents = 3; tokens = 2000; cost = 0.02
        capabilities = ["planning", "execution", "validation"]; reasoning = "Complex task requiring multiple specialized agents"
    else:
        mode = "standard"; score = 0.4; agents = 1; tokens = 300; cost = 0.003
        capabilities = ["general"]; reasoning = "Standard task with moderate complexity"
...
async def analyze_complexity(request: Request, body: ComplexityRequest, debug: Optional[bool] = False):
...
class TaskAnalysisResponse(BaseModel):
    task_type: str = Field(..., description="Type of task: Query, Analysis, Generation, Transformation, Execution, Unknown")
    complexity_score: float = Field(..., ge=0.0, le=1.0, description="Complexity score between 0 and 1")
    key_entities: List[str] = Field(default_factory=list, description="Key entities extracted from the task")
    ...
    success_criteria: List[str] = Field(default_factory=list, description="Success criteria for the task")
    reasoning: str = Field(default_factory=str, description="Reasoning for the analysis")
    source: str = Field(default="heuristic", description="'model' when provider used, else 'heuristic'")
...
def _heuristic_task_analysis(task: str) -> TaskAnalysisResponse:
    ...
    # Classify task type
    if any(word in lower_task for word in ["what", "who", "when", "where", "why", "how"]):
        task_type = "Query"
    elif any(word in lower_task for word in ["analyze", "compare", "evaluate", "assess"]):
        task_type = "Analysis"
    elif any(word in lower_task for word in ["create", "write", "generate", "build", "make"]):
        task_type = "Generation"
    elif any(word in lower_task for word in ["convert", "transform", "translate", "change"]):
...
async def analyze_task(request: Request, body: TaskAnalysisRequest):
    ...
    try:
        ...
        if task_type not in ["Query", "Analysis", "Generation", "Transformation", "Execution", "Unknown"]:
            task_type = "Unknown"
