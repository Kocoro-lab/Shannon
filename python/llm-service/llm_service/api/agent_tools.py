...
class AgentQueryWithTools(BaseModel):
    ...
    available_tools: Optional[List[str]] = Field(default=None, description="List of tools available to agent")
...
class AgentResponseWithTools(BaseModel):
    ...
    tool_calls: List[ToolCall] = Field(default_factory=list, description="Tools the agent wants to call")
...
async def agent_query_with_tools(request: Request, query: AgentQueryWithTools):
    ...
    try:
        ...
        if query.available_tools:
            available_tools = [
                tool for tool in query.available_tools 
...
async def _analyze_query_for_tools(query: str, available_tools: List[str]) -> List[ToolCall]:
    """
    ...
    if "calculator" in available_tools:
        ...
        math_keywords = ["calculate", "compute", "+", "-", "*", "/", "sqrt", "sum", "average"]
    ...
    if "web_search" in available_tools:
        search_keywords = ["search", "find", "look up", "google", "what is", "who is", "when was"]
    ...
    if "file_read" in available_tools and ("read" in query_lower or "open" in query_lower):
        ...
        for result in tool_results:
            if result["success"]:
                if result["tool"] == "calculator":
                    response_parts.append(f"The calculation result is: {result['output']}")
                    ...
                    response_parts.append(f"Tool {result['tool']} returned: {result['output']}")
            else:
                response_parts.append(f"Tool {result['tool']} failed: {result.get('error', 'Unknown error')}")
