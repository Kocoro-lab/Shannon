    ...
    for it.HasNext() {
        ...
        case enumspb.EVENT_TYPE_ACTIVITY_TASK_SCHEDULED:
            if a := e.GetActivityTaskScheduledEventAttributes(); a != nil {
                ...
                if mode == "full" { add("ACT_SCHEDULED", fmt.Sprintf("Activity %s(id=%s) scheduled", a.GetActivityType().GetName(), a.GetActivityId()), ts, uint64(e.GetEventId())) }
            }
        case enumspb.EVENT_TYPE_ACTIVITY_TASK_STARTED:
            if a := e.GetActivityTaskStartedEventAttributes(); a != nil {
                if st := acts[a.GetScheduledEventId()]; st != nil { st.Started = ts }
                if mode == "full" { add("ACT_STARTED", fmt.Sprintf("Activity started (scheduled_id=%d)", a.GetScheduledEventId()), ts, uint64(e.GetEventId())) }
            }
        ...
        case enumspb.EVENT_TYPE_TIMER_STARTED:
            if a := e.GetTimerStartedEventAttributes(); a != nil {
                ...
                if mode == "full" { add("TIMER_STARTED", fmt.Sprintf("Timer %s started for %s", a.GetTimerId(), a.GetStartToFireTimeout().AsDuration()), ts, uint64(e.GetEventId())) }
            }
        ...
        case enumspb.EVENT_TYPE_START_CHILD_WORKFLOW_EXECUTION_INITIATED:
            if a := e.GetStartChildWorkflowExecutionInitiatedEventAttributes(); a != nil {
                ...
                if mode == "full" { add("CHILD_INITIATED", fmt.Sprintf("Child %s(id=%s) initiated", a.GetWorkflowType().GetName(), a.GetWorkflowId()), ts, uint64(e.GetEventId())) }
            }
        case enumspb.EVENT_TYPE_CHILD_WORKFLOW_EXECUTION_STARTED:
            if a := e.GetChildWorkflowExecutionStartedEventAttributes(); a != nil {
                if c := childs[a.GetInitiatedEventId()]; c != nil { c.Started = ts; c.RunID = a.GetWorkflowExecution().GetRunId() }
        ...
        case enumspb.EVENT_TYPE_UPSERT_WORKFLOW_SEARCH_ATTRIBUTES:
            if mode == "full" { add("ATTR_UPSERT", "Search attributes upserted", ts, uint64(e.GetEventId())) }
        case enumspb.EVENT_TYPE_MARKER_RECORDED:
            if mode == "full" { add("MARKER_RECORDED", fmt.Sprintf("Marker recorded: %s", markerName(e)), ts, uint64(e.GetEventId())) }
        default:
            if mode == "full" {
...
func summarizeFailure(f *failurepb.Failure, includePayloads bool) string {
    ...
    reason := f.GetMessage()
    if !includePayloads && len(reason) > 200 { reason = reason[:200] + "…" }
    return reason
}
...
func childDuration(c *child, end time.Time) time.Duration {
    if c == nil || c.Started.IsZero() { return 0 }
    return end.Sub(c.Started)
}

func ctype(c *child) string { if c==nil {return "?"}; return c.Type }
func cid(c *child) string { if c==nil {return "?"}; return c.ID }

func safeChildType(childs map[int64]*child, id int64) string {
    if c := childs[id]; c != nil { return c.Type }
    return "?"
}

func safeChildID(childs map[int64]*child, id int64) string {
    if c := childs[id]; c != nil { return c.ID }
    return "?"
}

func markerName(e *historypb.HistoryEvent) string {
    if a := e.GetMarkerRecordedEventAttributes(); a != nil { return a.GetMarkerName() }
