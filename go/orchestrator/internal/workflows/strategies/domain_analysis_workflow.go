...
func buildDomainAnalysisDigestQuery(query, canonicalName string, planHints []string) string {
	...
	b.WriteString("- Include an \"Official Domains Coverage\" table with columns: domain, role, region, status\n")
	b.WriteString("- Organize findings by site role when present (Corporate/Docs/IR/Store)\n")
	b.WriteString("- Preserve distinct facts per domain; keep dates/numbers\n")
	b.WriteString("- Do NOT mention tools, workflows, or agents\n")
	if len(planHints) > 0 {
		b.WriteString("\nPlan hints:\n- ")
