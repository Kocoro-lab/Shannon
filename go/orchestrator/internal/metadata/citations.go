...
type Citation struct {
	...
	EntityRelevanceScore  float64    `json:"entity_relevance_score"`     // entity-specific relevance (0.0-1.0)
...
// ComputeEntityRelevance calculates entity-specific relevance score (0.0-1.0)
// based on domain matching, alias presence in title/snippet/source, and URL matching.
//
// Scoring (soft, additive, clamped 0-1):
//   - eTLD+1 or subdomain match with official_domains: 0.6
//   - Word-boundary alias in title: 0.25
//   - Alias in snippet: 0.15
//   - Alias in source: 0.1
//   - Alias in URL path: 0.1 (optional)
//
// Weights are configurable via weights map. Default weights used if not provided.
func ComputeEntityRelevance(c Citation, canonicalName string, aliases []string, officialDomains []string, weights map[string]float64) float64 {
	if canonicalName == "" {
		return 0.0
	}
	...
	if weights != nil {
		...
		if w, ok := weights["title"]; ok {
			titleWeight = w
		}
		if w, ok := weights["snippet"]; ok {
			snippetWeight = w
		}
		if w, ok := weights["source"]; ok {
			sourceWeight = w
		}
		if w, ok := weights["url"]; ok {
			urlWeight = w
		}
	}
	...
	for _, a := range aliases {
		...
		normalized = strings.Trim(normalized, "\"") // Remove quotes
		if normalized != "" && len(normalized) > 2 { // Skip very short aliases
	...
	for _, d := range officialDomains {
		...
		if normalized != "" {
			...
			// Also add eTLD+1 variants
			parts := strings.Split(normalized, ".")
			if len(parts) >= 2 {
				etld1 := strings.Join(parts[len(parts)-2:], ".")
				domainSet[etld1] = true
			}
		}
	}

	// Check 1: Domain match (eTLD+1 or subdomain)
	urlLower := strings.ToLower(c.URL)
	sourceLower := strings.ToLower(c.Source)
	for domain := range domainSet {
		if strings.Contains(urlLower, domain) || strings.Contains(sourceLower, domain) {
			score += domainWeight
			break
		}
	}

	// Check 2: Title contains alias (word-boundary matching preferred)
	titleLower := strings.ToLower(c.Title)
	for alias := range aliasSet {
		if wordBoundaryMatch(titleLower, alias) {
			score += titleWeight
			break
		}
	}

	// Check 3: Snippet contains alias
	snippetLower := strings.ToLower(c.Snippet)
	for alias := range aliasSet {
		if wordBoundaryMatch(snippetLower, alias) {
			score += snippetWeight
			break
		}
	}

	// Check 4: Source contains alias
	for alias := range aliasSet {
		if wordBoundaryMatch(sourceLower, alias) {
			score += sourceWeight
			break
		}
	}

	// Check 5: URL path/host contains alias
	for alias := range aliasSet {
		if strings.Contains(urlLower, alias) {
			score += urlWeight
			break
		}
	}
...
// wordBoundaryMatch checks if needle appears in haystack with word boundaries.
// Falls back to substring match for CJK characters (no word boundaries).
func wordBoundaryMatch(haystack, needle string) bool {
	if haystack == "" || needle == "" {
		return false
	}

	// For CJK characters, use simple contains
	if containsCJK(needle) {
		return strings.Contains(haystack, needle)
	}

	// For Latin/ASCII, use word boundary regex
...
// containsCJK checks if string contains CJK (Chinese/Japanese/Korean) characters
...
// RerankByEntity reranks citations by entity relevance combined with base quality.
//
// Ranking formula: base * (1 + alpha * EntityRelevanceScore)
//   where base = quality * credibility
//   and alpha controls the strength of entity boosting (default 0.5)
//
// Higher alpha = stronger entity preference
// alpha = 0 → pure quality×credibility ranking
// alpha = 1 → balanced entity+quality ranking
// alpha > 1 → strong entity preference
//
// Stable tie-breaking by normalized URL for determinism.
func RerankByEntity(citations []Citation, canonicalName string, aliases []string, officialDomains []string, alpha float64, weights map[string]float64) []Citation {
	if len(citations) == 0 || canonicalName == "" {
		return citations
	}
	...
	// Compute entity relevance scores for all citations
	scored := make([]struct {
		citation    Citation
		baseScore   float64
		finalScore  float64
	...
	for i, c := range citations {
		...
		entityScore := ComputeEntityRelevance(c, canonicalName, aliases, officialDomains, weights)
		c.EntityRelevanceScore = entityScore

		// Base score: quality × credibility
		base := c.QualityScore * c.CredibilityScore

		// Final score: base * (1 + alpha * entityRelevance)
		final := base * (1 + alpha*entityScore)

		// Normalize URL for stable tie-breaking
		normalizedURL := strings.ToLower(strings.TrimSpace(c.URL))
	...
	// Sort by final score (descending), then by normalized URL (ascending) for stability
	sort.Slice(scored, func(i, j int) bool {
		if scored[i].finalScore != scored[j].finalScore {
			return scored[i].finalScore > scored[j].finalScore
		}
		return scored[i].normalizedURL < scored[j].normalizedURL
	})

	// Extract sorted citations
	result := make([]Citation, len(scored))
