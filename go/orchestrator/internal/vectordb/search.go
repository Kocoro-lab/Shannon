...
func (c *Client) FindSimilarQueries(ctx context.Context, embedding []float32, limit int) ([]SimilarQuery, error) {
    ...
    for _, p := range pts {
        ...
        if q, ok := p.Payload["query"].(string); ok { sq.Query = q }
        if o, ok := p.Payload["outcome"].(string); ok { sq.Outcome = o }
        if ts, ok := p.Payload["timestamp"].(float64); ok { sq.Timestamp = time.Unix(int64(ts), 0) }
        out = append(out, sq)
    }
    return out, nil
}
