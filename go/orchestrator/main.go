...
func main() {
        ...
        if err := shannonCfgMgr.Initialize(); err != nil {
			...
			if shCfg := shannonCfgMgr.GetConfig(); shCfg != nil {
				...
				// Initialize embedding service and vectordb from Shannon config
				{
					...
					if base == "" { base = getEnvOrDefault("LLM_SERVICE_URL", "http://llm-service:8000") }
					...
					if ecfg.EnableRedis {
						if c, err := embeddings.NewRedisCache(ecfg.RedisAddr); err == nil { cache = c } else { logger.Warn("Embeddings Redis cache init failed", zap.Error(err)) }
					}
    ...
    go func() {
        ...
        for attempt := 1; ; attempt++ {
            ...
            if err == nil { break }
            delay := time.Duration(attempt); if delay > 15 { delay = 15 }
